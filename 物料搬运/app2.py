...
def parse_assembly_components(file_path):
    """
    解析Excel文件，提取装配件及其子组件的替代关系
    
    参数:
        file_path: Excel文件路径
        
    返回:
        字典结构: {装配件: {子组件: [一组替代料列表]}}
    """
    # 存储结果的字典
    result = {}
    ...
    for sheet_name in excel_file.sheet_names:
        ...
        # 确保必要的列存在
        required_columns = ['装配件', '装配件产品中类', '装配件描述', '层级', '序号', '子组件']
        if not set(required_columns).issubset(df.columns):
            print(f"工作表 '{sheet_name}' 缺少必要的列，已跳过")
            continue
        ...
        for _, row in df.iterrows():
            ...
            # 检查序号是否表示替代料（假设以"替代"结尾）
            is_alternative = str(seq_number).strip().endswith('替代')
            ...
            if is_alternative:
                # 寻找对应的主组件（上一个非替代料）
                # 这里简化处理，实际可能需要更复杂的逻辑来匹配主组件
                main_components = list(result[assembly].keys())
                if main_components:
                    main_component = main_components[-1]
                    if component not in result[assembly][main_component]:
                        result[assembly][main_component].append(component)
            else:
                # 为主组件初始化替代料列表
                if component not in result[assembly]:
                    result[assembly][component] = []
                # 将自身添加到列表首位
                if not result[assembly][component]:
                    result[assembly][component].append(component)
...
def process_excel_files(file2, file3, project_list, big_plan_path):
    ...
    group=parse_assembly_components(big_plan_path)
    # group = {}
    next_id = -1
    ...
    try:
        ...
        for _, row in df3.iterrows():
            ...
            if key2 not in group or row.iloc[7] not in group[key2]:
                continue
            flag = 0
            for j in group[key2][row.iloc[7]]:
                if j in result_dict:
                    flag = 1
            if flag == 0:
                continue
