...
#     """
#     彻底清理Windows文件名中的所有非法字符（包括可见非法字符和不可见控制字符）
...
#     # 处理倒序（如J-A自动转为A-J，保证范围正确，但不改变整体列表顺序）
...
#     """
#     批量解析列列表（列表内包含范围字符串/单列字符串）
#     核心修复：仅按原始顺序去重，不自动排序，保留用户配置的对应关系
#     :param col_input_list: 输入列列表，如["A-J", "L", "P-R"]
#     :return: 扁平化后的完整列字母列表（保留原始配置顺序，仅去重）
#     """
...
#     # 关键修复：按原始顺序去重（保留第一次出现的元素，剔除后续重复项），不进行排序
#     seen = set()
...
#     """
#     辅助函数：解析列列表为索引列表（兼容原有逻辑）
#     """
#     # 第一步：批量解析列表，得到扁平化列字母列表（保留原始顺序）
...
#             # 情况2：列标识是字母（如"A"→1，"AB"→28）
#             else:
#                 col_idx = column_index_from_string(col_identifier)
            
#             # 验证列索引有效性
#             if col_idx < 1:
#                 raise ValueError(f"无效的列索引：{col_idx}，必须大于0")
#             col_indexes.append(col_idx)
#         except Exception as e:
#             # 兜底：通过表头匹配列名（如"物料编码"→对应列索引）
#             try:
#                 col_idx = None
#                 if file_format == "xlsx":
#                     for col in ws.iter_cols(min_row=header_row, max_row=header_row):
...
#         local_rule.setdefault("force_formula_cols", [])  # 新增：默认空列表，兼容无配置情况
        
#         # ========== 核心校验：cols_range 是列表，且项为字符串 ==========
#         if "cols_range" not in rule["attach_rule"] or "cols_range" not in rule["local_rule"]:
#             raise KeyError(f"第{i+1}条规则缺失cols_range配置")
        
#         # 校验 attach_rule 的 cols_range
#         attach_col_list = rule["attach_rule"]["cols_range"]
#         if not isinstance(attach_col_list, list):
#             raise ValueError(f"第{i+1}条规则attach_rule的cols_range必须是列表，当前类型：{type(attach_col_list)}")
#         if len(attach_col_list) == 0:
#             raise ValueError(f"第{i+1}条规则attach_rule的cols_range列表不能为空")
#         # 验证列表内每个项，并提前解析校验有效性
#         try:
...
#         # 校验 local_rule 的 cols_range
#         local_col_list = local_rule["cols_range"]
#         if not isinstance(local_col_list, list):
#             raise ValueError(f"第{i+1}条规则local_rule的cols_range必须是列表，当前类型：{type(local_col_list)}")
#         if len(local_col_list) == 0:
#             raise ValueError(f"第{i+1}条规则local_rule的cols_range列表不能为空")
#         # 验证列表内每个项，并提前解析校验有效性
#         try:
...
#         # 额外校验：解析后的附件列数和本地列数必须一致
#         if len(parsed_attach_cols) != len(parsed_local_cols):
#             raise ValueError(f"第{i+1}条规则：附件列解析后({len(parsed_attach_cols)}列)与本地列解析后({len(parsed_local_cols)}列)数量不一致")
...
#         filtered_encoded_folders = filter_folders(target_encoded_folders, email_config["exclude_folders"], all_folders)
...
# # ===================== 按列标识提取数据（保留原始顺序） =====================
# def extract_cols_from_attach(attach_path: str, attach_rule: Dict[str, Any]) -> Dict[str, List[Any]]:
#     """
#     按列标识（如"D"）提取数据，返回 {列标识: 数据列表}，严格保留配置原始顺序
#     """
#     sheet_name = attach_rule["sheet"]
#     header_row = attach_rule["header_row"]
#     col_list = attach_rule["cols_range"]  # 配置的列标识列表（如["D", "G", "H"]）
...
#         # 第一步：解析配置列列表，保留原始顺序（仅去重）
#         full_col_letter_list = parse_col_list(col_list)
#         print(f"\n📊 提取附件列（共{len(full_col_letter_list)}列，严格保留配置顺序）：")
...
#         # 第二步：按列标识提取数据，建立 {列标识: 数据列表} 映射（保留原始顺序）
#         extract_data = {}
#         for col_letter in full_col_letter_list:
#             # 获取列索引（按列标识匹配，不依赖物理顺序）
#             col_idx = get_col_index_compatible(ws, col_letter, header_row, real_format)
#             col_data = []
#             for row in valid_rows:
#                 cell_value = get_cell_value(ws, row, col_idx, real_format)
#                 col_data.append(cell_value)
#             extract_data[col_letter] = col_data  # 键为列标识（如"D"），值为数据列表
...
#         print(f"✅ 精准清空{len(col_letter_list)}个配置列的旧数据行（其他列完全保留）")

#     # 通用配置
#     sheet_name = local_rule["sheet"]
#     header_row = local_rule["header_row"]
#     local_col_list = local_rule["cols_range"]  # 本地列配置列表
...
#         # 打印本地列配置信息
#         print(f"\n📊 本地目标列（共{len(local_full_col_letter_list)}列，严格保留映射顺序）：")
...
#         print(f"\n🚀 开始批量写入{data_rows_count}行数据（按列标识精准写入）...")

#         # 按映射关系写入数据：附件列 → 本地列（严格保留配置顺序）
#         for attach_col, local_col in col_mapping.items():
...
#         print(f"   核心保障：✅ 按配置原始顺序映射 ✅ 精准写入对应列 ✅ 保留所有业务逻辑")
#         return True
...
#     # 请修改为你的配置文件实际路径
#     main(config_path="D:\\供应链\\芯片\\configpdv6.json")
