...
def process_excel_files(file1,file2,file3,path): #每天更新
    # 记录开始时间
...
def updatebig_plan(source_file, target_file,qmonth): # 每月更新
    # 获取当前年月
    now = datetime.now()
    year = now.year
    month = qmonth#now.month
    
    # 创建新文件名称
    new_file = f'E:\\供应链\\总表\\101225\\电子料总欠料表{year}年{month}月.xlsx'
    
    # 读取源文件中的量产欠料sheet，跳过前四行，第五行作为表头
    source_df = pd.read_excel(source_file, sheet_name='量产欠料 ', header=17)
    ...
    column_mapping = {
        0: 2,  # 目标第2列(index=1) <- 源第2列(index=1)
        1: 3,  # 目标第3列(index=2) <- 源第3列(index=2)
        2: 4,  # 目标第4列(index=3) <- 源第4列(index=3)
        3: 6,  # 目标第5列(index=4) <- 源第6列(index=5)
        7: 8,  # 目标第9列(index=8) <- 源第8列(index=7)
    }
    
    # 如果找到匹配的列，则添加第18列的映射
    if current_month_column is not None:
        source_col_idx = source_df.columns.get_loc(current_month_column)
        column_mapping[21] = source_col_idx  # 目标第18列(index=17)
...
def update_schedue(source_file, target_file,path): # 每周更新
    # 获取当前年月
    now = datetime.now()
    year = now.year
    month = now.month
    ...
    print(day)
    # print(base_date,now)
    # print(int(now-base_date))
    time=(now-base_date).days+13-day
    print(time)
    # print(time/60/60/24)
    # 创建新文件名称
    new_file = path+'交期_'+target_file
    target_file=path+target_file
    # 读取源文件中的量产欠料sheet，跳过前四行，第五行作为表头
    result=process_material_data(source_file,time)
    ...
    for row_idx in range(6,len(target_df)):
        ...
        if ws.cell(row=row_idx, column=2).value in result.keys():
            ...
            ws.cell(row=row_idx, column=23).value=f"'{value_to_save}"
            ws.cell(row=row_idx, column=23).number_format = '@'
            print(ws.cell(row=row_idx, column=23).value)
...
def is_float(value: str) -> bool:
    try:
        ...
        return 0
def process_material_data(file_path,times):
    # 读取 Excel 文件并获取所有表名
    ...
    for col in df.columns :
        print(type(col))
    print(df.columns )
    date_columns = [int(col if type(col) in [int, float]else col.timestamp()) for col in df.columns if type(col) in [int, float,datetime]]
    print(date_columns)
    sum={}
    for index, row in df.iterrows():
        material_code = row['物料编码']
        ...
        for date_col in date_columns:
            ...
            if(date_col<=times):
                ...
                if (type(value)==str or not np.isnan(value)):
                    ...
                    sum[material_code]+=is_float(value)
            if pd.notnull(value):
                result_dict[material_code].append({(base_date+timedelta(days=date_col)).strftime('%Y-%m-%d'): value})
        if(material_code=='Y03240051301'):
            print("tttttt",result_dict[material_code])
    print(result_dict)
    print(sum)
    # import pdb;pdb.set_trace()
    for i in sum.keys():
        result_dict[i].append(sum[i])
    return result_dict
...
if __name__ == "__main__":
    # process_excel_files("E:\\供应链\\总表\\101225\\电子料总欠料表20251209.xlsx", "E:\\供应链\\总表\\101225\\BHSC_库存收发存报表(不_101225.xls", "E:\\供应链\\总表\\101225\\BHSC_外协厂投料明细表(_101225.xls","E:\\供应链\\总表\\101225")
    # 
    # updatebig_plan("E:\\供应链\\总表\\101225\\需求数据--2025.12.06 vs 备料大计划1112 V1.6版.xlsx","E:\\供应链\\总表\\081225\电子料总欠料表20251208.xlsx",12)    
    ...
    update_schedue("E:\\供应链\\总表\\101225\\0425交期回复 (1)秋梅.xlsx","电子料总欠料表20251209.xlsx","E:\\供应链\\总表\\101225\\")    
