...
def process_first_excel(file):
    ...
    for index, row in df.iterrows():
        ...
        tmp_dict=(row.iloc[5],row.iloc[12])
...
tmp_mer={}
def check(order_id,z_id,tot,id):
    main_num=1
    ...
    if(id in bom_dict[z_id]):
        ...
        for i in bom_dict[z_id][id]:
            ...
            if(i[0] in tmp_mer[order_id]):
                if(order_id=='DGAH20250429002-Z(YZ)' and i[0]=='Y02122612M01'):
                    print(z_id,id)
                    print(order_id)
                    print("bom",bom_dict[z_id][id][0][1])
                    print(i,tmp_mer[order_id])
                ...
                if(i[1] and tmp_mer[order_id][i[0]]/i[1]*bom_dict[z_id][id][0][1]>=tot_need):
                    ...
                    tot_need-=tmp_mer[order_id][i[0]]/i[1]*bom_dict[z_id][id][0][1]
    ok=0
    # print(next_id[z_id])
    if(id in next_id[z_id]):
        ok=1 
        # print(z_id)
        # if(z_id=="C06010836"):
        #     print("uuuu",id)
        ...
        for i in next_id[z_id][id]:
            if(z_id=="C06010705" and i=='S.10.220'):
                print("uuuu",id)
                print(next_id[z_id][id])
                print(next_id[z_id])
            ...
            ok&=check(order_id,z_id,tot_need/main_num,i)
    return ok
def process_second_excel(file, first_dict):
    ...
    for index, row in df.iterrows():
        ...
        if(row.iloc[0][-2:]=='-B' or row.iloc[0][-6:]=='-B(YZ)'  or row.iloc[0][-6:]=='-B(YZ）' or row.iloc[0][-7:]=='-B1(YZ)' or row.iloc[0][-4:]=='(CG)' or row.iloc[0][-6:]=='(YZCG)'):
            if(row.iloc[1] == row.iloc[12]):
                tmp_mer[row.iloc[0]][row.iloc[12]]=0
            if tmp_mer[row.iloc[0]][row.iloc[12]]>0 and row.iloc[0] not in is_processd:
                is_processd[row.iloc[0]]=1
                check(row.iloc[0],row.iloc[1],row.iloc[3],'S')
            continue
    ...
    "投料有差异，请完成投料",
        ...
        np.where(  # 第三个条件：X不等于-9999999时，判断X的绝对值是否小于0.99
            abs(df['需求量差']) < 0.99999,
            "OK,可关闭",
            "需求差异"
        )
